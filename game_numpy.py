...
class VI_simplex_product:
    def __init__(self, VI_F):
        self.VI_F = VI_F

    def solve(self, iter_init, gap_tol, efficient_corrector=True, maxnit=50000, maxsubnit=10, verbose=0, record_file=None, print_preci=6):
        ...
        self.m, self.n, self.mn = *iter_init.shape, iter_init.shape[0]*iter_init.shape[1]
        ...
        try:
            pi = self.path_following(iter_init)
    ...
    def corrector_comp(self, pi, tau):
    ...
    def predictor_corrector(self, pi, tau_isum, enforce_decrease=False):
        F, r_difference, tau_check_isum, I_1pi, J_pi = self.state_comp(pi)
        ...
        J_G = J_pi+np.diag(tau_isum.repeat(self.n))
        dtau_sign = 1 if enforce_decrease else np.linalg.slogdet(J_G)[0]
        ...
        for subnit in range(self.maxsubnit):
            ...
            if (corrected := G_pi_tau_norm <= self.tau_min):
                break
            _pi = (lambda vec: vec/vec.sum(axis=-1, keepdims=True))(np.exp(np.log(pi)-corrector_pi))
            pi = _pi if not np.isnan(_pi).any() else pi
            self.nit += 1
        if not corrected:
