# fmt: off
import os
# remove the following three lines if only single process is used
os.environ["OMP_NUM_THREADS"] = "1"
os.environ["OPENBLAS_NUM_THREADS"] = "1"
os.environ["MKL_NUM_THREADS"] = "1"
import numpy as np
from multiprocessing import Pool
from vi_numpy import VI_simplex
from game_numpy import VI_simplex_product
# fmt: on
...
class nn_F:
    def __init__(self, seed, nn_layers, activation):
        np.random.seed(seed)
        self.ws = [np.random.normal(loc=0.0, scale=1/np.sqrt((nn_layers[i]+nn_layers[i+1])/2), size=(nn_layers[i+1], nn_layers[i])) for i in range(len(nn_layers)-1)]
        self.bs = [np.random.normal(loc=0.0, scale=1e-2, size=nn_layers[i+1]) for i in range(len(nn_layers)-1)]
...
class payoff_F:
    ...
    def value_Jaco(self, pi):
        ...
        J = np.matmul(c.transpose(1, 2, 0)[:, :, None, :]*U_ii.transpose(1, 2, 0)[:, None, :, :], self.payoff.transpose(3, 1, 0, 2)).transpose(0, 2, 1, 3)
        return F, J
...
def test_vi(seed, n, hidden_layers, verbose=0):
    VI_problem = nn_F(seed, [n, *hidden_layers, n], 0)

    np.random.seed(seed)
    sigma_init = np.random.dirichlet(np.ones(n))
    result_sigma, record_list = VI_simplex(VI_problem).solve(sigma_init, gap_tol=1e-5, maxnit=50000, maxsubnit=10, verbose=verbose, record_file=frecord, print_preci=print_preci)
    return result_string(seed, result_sigma, record_list)


def test_game(seed, dimension, hidden_layers, verbose=0):
    m, n = dimension
    if not (payoff_game := True):
        Game_problem = nn_F(seed, [m*n, *hidden_layers, m*n], 0)
    else:
        Game_problem = payoff_F(seed, m, n, hidden_layers[0])

    np.random.seed(seed)
    pi_init = np.random.dirichlet(np.ones(n), size=(m,))
    result_pi, record_list = VI_simplex_product(Game_problem).solve(pi_init, gap_tol=1e-5, maxnit=50000, maxsubnit=10, verbose=verbose, record_file=frecord, print_preci=print_preci)
    return result_string(seed, result_pi, record_list)


def group_test(test_fun, dim, hidden_layers, testrange):
    ...
    with open(fresult, 'w') as f:
        ...
        f.writelines(f"|{'seed':^5}|{'success':^7}|{'nit':^7}|{'gap':^{print_len}}|{'tau_sum':^{print_len}}|{'G_norm':^{print_len}}|{'avoid_n':^7}|\n")
    with Pool(process_num) as pool:
        async_results = [pool.apply_async(test_fun, args=(i, dim, hidden_layers), callback=write_result) for i in testrange]
...
if __name__ == '__main__':
    ...
    n = 16
    hidden_layers = [n]
    testrange = np.arange(2)
    # group_test(test_game, (m, n), hidden_layers, testrange)
    # group_test(test_vi, n, hidden_layers, testrange)
    seed = 0
    # print(test_game(seed, (m, n), hidden_layers, verbose=1))
    print(test_vi(seed, n, hidden_layers, verbose=1))
